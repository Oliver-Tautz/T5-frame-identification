...
MODEL_FILENAME = args.model_savename
EPOCHS = args.epochs
...
def train_and_save(pretrained_name,X,X_val,epochs=5,batch_size=6,learning_rate=1e-4):
    ...
    model.save_pretrained("first_try")
...
def evaluate(model, args_test,history , batch_size=BATCHSIZE):
    ...
    for i, (pred, label) in enumerate(zip(predictions, y_test)):
        ...
        if pred == "":
            ...
            predictions[i] = "EMPTY"
    print(rouge.get_scores(predictions, y_test, avg=True)['rouge-1']['f'])
...
if __name__ == "__main__":
    ...
    trained_model, history = train_and_save('t5-small', X_train, X_val, batch_size=BATCHSIZE, epochs=EPOCHS)
